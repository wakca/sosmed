<div class="panel panel-default">
    <div class='panel-heading'><a href='/{{ $receiver->username }}'><strong>{{ $receiver->name }}</a></strong> <span class='small-text'>{{ "<".$receiver->username.">" }}</span></div>
...
<div class='panel panel-default'>
    <div class='panel-body'>
        <form id='sendForm' action='/messages' class="form-horizontal" method='POST' enctype="multipart/form-data">
            {{ csrf_field() }}
            <textarea class="form-control" id='message' name='message' rows='3' placeholder="Masukan Pesan anda" required></textarea>
            <input type='hidden' id='user_id' name='user_id' value='{{ Auth::Id() }}'/>
            <input type='hidden' id='recepient_id' name='recepient_id' value='{{ $receiver->id }}'/>
    ...
    <script type="text/javascript">
        ...
        var options = {
            ...
            function getNewMessage() {
                $("#message-container").load('/messages/{{$receiver->username}}/list',function(){
                    updateScroll()
                });
            }
    </script>
